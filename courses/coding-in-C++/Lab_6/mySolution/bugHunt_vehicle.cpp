...
void Vehicle::accelerate(std::double_t amount_kmh)
{
    try
    {
        if (amount_kmh <= 0.0)
...
void Vehicle::brake(std::double_t amount_kmh)
{
    if (amount_kmh <= 0.0)
    {
        return;
...
if (speed_kmh < 0.0)
{
    // speed_kmh = amount_kmh; // If speed bekomes negative, speed is set to the amount it should breake, not 0.0
    speed_kmh = 0.0;
}
}

void Vehicle::steer(std::double_t angle)
{
    steering_angle = angle;
}

void Vehicle::update_lane_offset(std::double_t offset)
{
    lane_offset_m = offset;
}

std::double_t Vehicle::get_speed() const
{
    return speed_kmh;
}

std::double_t Vehicle::get_steering_angle() const
{
    return steering_angle;
}

std::double_t Vehicle::get_lane_offset() const
{
